...
import HelpModal from '../../components/help/HelpModal3';
...
import wandImage from '../../assets/images/wand3.png';
...
const MakeAWishScreen3 = ({ selectedLanguage }) => {
  ...
  const storedStickers = JSON.parse(localStorage.getItem('wishdroppedStickers3')) || [];
  const storedSliders = JSON.parse(localStorage.getItem('wishdroppedSliders3')) || [];
  const storedActions = JSON.parse(localStorage.getItem("wishactionHistory3")) || [];
  const storedNotes = JSON.parse(localStorage.getItem("wishdroppedNotes3")) || [];  // New state for notes
  const [selectedCategory, setSelectedCategory] = useState('');
  const [wishactionHistory3, setActionHistory] = useState(storedActions)
  const [wishdroppedStickers3, setDroppedStickers] = useState(storedStickers);
  const [wishdroppedSliders3, setDroppedSliders] = useState(storedSliders);
  const [wishdroppedNotes3, setDroppedNotes] = useState(storedNotes);  // New state for notes

  const [showHelpModal, setShowHelpModal] = useState(false);

  function saveOnLeavePage(){
    localStorage.setItem('wishactionHistory3', JSON.stringify(wishactionHistory3));
    localStorage.setItem('wishdroppedNotes3', JSON.stringify(wishdroppedNotes3));  
  }
  ...
  useEffect(() => {
    localStorage.setItem('wishdroppedStickers3', JSON.stringify(wishdroppedStickers3));
    localStorage.setItem('wishactionHistory3', JSON.stringify(wishactionHistory3)); //Updates the saved wishactionHistory
  }, [wishdroppedStickers3]);
  useEffect(() => {
    localStorage.setItem('wishdroppedSliders3', JSON.stringify(wishdroppedSliders3));
    localStorage.setItem('wishactionHistory3', JSON.stringify(wishactionHistory3));
  }, [wishdroppedSliders3]);
  useEffect(() => {
    localStorage.setItem('wishdroppedNotes3', JSON.stringify(wishdroppedNotes3));
  ...
  const handleRightButtonClick = () => {
    ...
    setTimeout(async () => {
      ...
      if (container) {
        ...
        const wishscreenshot3 = canvas.toDataURL();
        localStorage.setItem('wishScreenshot3', wishscreenshot3);
        saveOnLeavePage()
        navigate('/letter-to-judge');
      }
  ...
  const handleStickerDelete = (stickerId) => {
    wishactionHistory3.push(wishdroppedStickers3.findLast((sticker) => sticker.id === stickerId))
    setDroppedStickers((prevStickers) =>
  ...
  const handleSliderDrop = (updatedSlider) => {
    wishactionHistory3.push(wishdroppedSliders3.findLast((slider) => slider.id === updatedSlider))
    setDroppedSliders((prevSliders) =>
  ...
  const handleSliderDelete = (sliderId) => {
    wishactionHistory3.push(wishdroppedSliders3.findLast((slider) => slider.id === sliderId))
    setDroppedSliders((prevSliders) =>
  ...
  const btnUndo = () => {
    clickSound.play()
    let lastAction = wishactionHistory3.pop()
    ...
    if ("icon" in lastAction) { //Checks is about a sticker (has field icon)
      if ("x" in lastAction) { 
        ...
        let sticker = wishdroppedStickers3.find((sticker) => sticker.id === lastAction.id )
        if (sticker) { 
          ...
          setDroppedStickers([...wishdroppedStickers3, lastAction])
        }
      }
      ...
      if ("x" in lastAction) {
        let slider = wishdroppedStickers3.find((slider) => slider.id === lastAction.id )
        if (slider) {
          handleSliderDrop(lastAction)
        }
        else {
          setDroppedSliders([...wishdroppedSliders3, lastAction])
        }
      }
  ...
  const handleStroke = (e) => {
    ...
    if (e.paths.length !== 1) {
      wishactionHistory3.push("DrawStroke")
    }
  }
  ...
  return (
    <div className="make-wish"  onClick={handleScreenClick}>
      ...
      <h1 className="make-wish-title">
          ...
          <span className='make-wish-title-line2'>{t('this_is_my_wish_for_my_family')}</span>
          <span className='make-wish-title-line3'>{t('a_wish_for_my_family')}</span>
        </h1>
      </div>
      ...
      <div className="make-wish-avatar-display-container" ref={drop} style={{ position: 'relative' }}>
        {/* CANVAS FOR DRAWING */}
        <DrawingCanvas forwardRef={getCanvas} storagePath='wishuserDrawing3' handleStroke={handleStroke} pencilInfo={selectedPencil} key={selectedPencil} isActive={isDrawingMode}>
        ...
        {/* Stickers */}
        {wishdroppedStickers3.map((sticker) => (
          <DraggableSticker
            key={sticker.id}
            sticker={sticker}
        ...
        {wishdroppedSliders3.map((slider) => (
          <DraggableSlider
            key={slider.id}
            slider={slider}
...
export default MakeAWishScreen3;
