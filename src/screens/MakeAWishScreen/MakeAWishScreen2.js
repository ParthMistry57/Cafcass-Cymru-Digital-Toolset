...
import HelpModal from '../../components/help/HelpModal2';
...
import wandImage from '../../assets/images/wand2.png';
import Button from '../../components/button';
import { btnBlue1, makeAWish } from '../../assets/images/ui';
import ConfirmPopup from '../../components/ConfirmPopup';
import wizardHat from '../../assets/images/wizardHat.png';
...
const MakeAWishScreen2 = ({ selectedLanguage }) => {
  ...
  const storedStickers = JSON.parse(localStorage.getItem('wishdroppedStickers2')) || [];
  const storedSliders = JSON.parse(localStorage.getItem('wishdroppedSliders2')) || [];
  const storedActions = JSON.parse(localStorage.getItem("wishactionHistory2")) || [];
  const storedNotes = JSON.parse(localStorage.getItem("wishdroppedNotes2")) || [];  // New state for notes
  const [selectedCategory, setSelectedCategory] = useState('');
  const [wishactionHistory2, setActionHistory] = useState(storedActions)
  const [wishdroppedStickers2, setDroppedStickers] = useState(storedStickers);
  const [wishdroppedSliders2, setDroppedSliders] = useState(storedSliders);
  const [wishdroppedNotes2, setDroppedNotes] = useState(storedNotes);  // New state for notes

  const [showHelpModal, setShowHelpModal] = useState(false);

  function saveOnLeavePage(){
    localStorage.setItem('wishactionHistory2', JSON.stringify(wishactionHistory2));
    localStorage.setItem('wishdroppedNotes2', JSON.stringify(wishdroppedNotes2));  
  }
  ...
  useEffect(() => {
    localStorage.setItem('wishdroppedStickers2', JSON.stringify(wishdroppedStickers2));
    localStorage.setItem('wishactionHistory2', JSON.stringify(wishactionHistory2)); //Updates the saved wishactionHistory2
  }, [wishdroppedStickers2]);
  useEffect(() => {
    localStorage.setItem('wishdroppedSliders2', JSON.stringify(wishdroppedSliders2));
    localStorage.setItem('wishactionHistory2', JSON.stringify(wishactionHistory2));
  }, [wishdroppedSliders2]);
  useEffect(() => {
    localStorage.setItem('wishdroppedNotes2', JSON.stringify(wishdroppedNotes2));
  ...
  const handleRightButtonClick = () => {
    ...
    setTimeout(async () => {
      ...
      if (container) {
        ...
        const wishscreenshot2 = canvas.toDataURL();
        localStorage.setItem('wishScreenshot2', wishscreenshot2);
        saveOnLeavePage()
        navigate('/make-a-wish-family');
      }
  ...
  const handleStickerDelete = (stickerId) => {
    wishactionHistory2.push(wishdroppedStickers2.findLast((sticker) => sticker.id === stickerId))
    setDroppedStickers((prevStickers) =>
  ...
  const handleSliderDrop = (updatedSlider) => {
    wishactionHistory2.push(wishdroppedSliders2.findLast((slider) => slider.id === updatedSlider))
    setDroppedSliders((prevSliders) =>
  ...
  const handleSliderDelete = (sliderId) => {
    wishactionHistory2.push(wishdroppedSliders2.findLast((slider) => slider.id === sliderId))
    setDroppedSliders((prevSliders) =>
  ...
  const btnUndo = () => {
    clickSound.play()
    let lastAction = wishactionHistory2.pop()
    ...
    if ("icon" in lastAction) { //Checks is about a sticker (has field icon)
      if ("x" in lastAction) { 
        ...
        let sticker = wishdroppedStickers2.find((sticker) => sticker.id === lastAction.id )
        if (sticker) { 
          ...
          setDroppedStickers([...wishdroppedStickers2, lastAction])
        }
      }
      ...
      if ("x" in lastAction) {
        let slider = wishdroppedStickers2.find((slider) => slider.id === lastAction.id )
        if (slider) {
          handleSliderDrop(lastAction)
        }
        else {
          setDroppedSliders([...wishdroppedSliders2, lastAction])
        }
      }
  ...
  const handleStroke = (e) => {
    ...
    if (e.paths.length !== 1) {
      wishactionHistory2.push("DrawStroke")
    }
  }
  ...
  return (
    <div className="make-wish" onClick={handleScreenClick}>
      ...
      <h1 className="make-wish-title">
          ...
          <span className='make-wish-title-line2'>{t('this_is_my_wish_for_the_future')}</span>
          <span className='make-wish-title-line3'>{t('a_wish_for_the_future')}</span>
        </h1>
      </div>
      ...
      <div className="make-wish-avatar-display-container" ref={drop} style={{ position: 'relative' }}>
        {/* CANVAS FOR DRAWING */}
        <DrawingCanvas forwardRef={getCanvas} storagePath='wishuserDrawing2' handleStroke={handleStroke} pencilInfo={selectedPencil} key={selectedPencil} isActive={isDrawingMode}>
        ...
        {/* Stickers */}
        {wishdroppedStickers2.map((sticker) => (
          <DraggableSticker
            key={sticker.id}
            sticker={sticker}
        ...
        {wishdroppedSliders2.map((slider) => (
          <DraggableSlider
            key={slider.id}
            slider={slider}
...
export default MakeAWishScreen2;
