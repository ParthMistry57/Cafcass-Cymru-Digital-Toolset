import React, { useEffect, useState, useRef, useEffectOnce  } from 'react';
...
const DirectWorkScreen = ({ selectedLanguage }) => {
  ...
  const storedStickers = JSON.parse(localStorage.getItem('droppedStickers')) || [];
  const storedSliders = JSON.parse(localStorage.getItem('droppedSliders')) || [];
  const storedActions = JSON.parse(localStorage.getItem("actionHistory")) || [];
  const storedNotes = JSON.parse(localStorage.getItem("droppedNotes")) || [];  // New state for notes
  
  const [selectedCategory, setSelectedCategory] = useState('');
  const [actionHistory, setActionHistory] = useState(storedActions)
  const [droppedStickers, setDroppedStickers] = useState(storedStickers);
  const [droppedSliders, setDroppedSliders] = useState(storedSliders);
  const [droppedNotes, setDroppedNotes] = useState(storedNotes);  // New state for notes

  const [showHelpModal, setShowHelpModal] = useState(false);

  function saveOnLeavePage() {
    localStorage.setItem('actionHistory', JSON.stringify(actionHistory));
    localStorage.setItem('droppedNotes', JSON.stringify(droppedNotes));  // Save notes on leave
  }
  ...
  useEffect(() => {
    localStorage.setItem('droppedStickers', JSON.stringify(droppedStickers));
    localStorage.setItem('actionHistory', JSON.stringify(actionHistory));
  }, [droppedStickers]);
  useEffect(() => {
    localStorage.setItem('droppedSliders', JSON.stringify(droppedSliders));
    localStorage.setItem('actionHistory', JSON.stringify(actionHistory));
  }, [droppedSliders]);
  useEffect(() => {
    localStorage.setItem('droppedNotes', JSON.stringify(droppedNotes));
    localStorage.setItem('actionHistory', JSON.stringify(actionHistory));
  }, [droppedNotes]);
  ...
  const handleRightButtonClick = async () => {
    audio.play();
    const container = document.querySelector('.direct-work');
    if (container) {
      const canvas = await html2canvas(container);
      const screenshot = canvas.toDataURL();
      localStorage.setItem('directWorkScreenshot', screenshot);
      saveOnLeavePage()
  ...
  const handleDrop = (item, monitor) => {
    ...
    const container = document.querySelector('.direct-work-avatar-display-container');
    ...
    /** @note whomever adds the next items for the post-its and text boxes, make sure you include a inique identifier in the IF statement below. Otherwise other types of draggable objects will end up being places in the same spot at the same time */
  ...
  const handleScreenClick = (e) => {
    if (selectedCategory === "PostIt") {
      ...
      actionHistory.push({...newNote, creation: true})
  ...
  const handleStickerDelete = (stickerId) => {
    actionHistory.push(droppedStickers.findLast((sticker) => sticker.id === stickerId))
    setDroppedStickers((prevStickers) =>
  ...
  const handleSliderDrop = (updatedSlider) => {
    actionHistory.push(droppedSliders.findLast((slider) => slider.id === updatedSlider))
    setDroppedSliders((prevSliders) =>
  ...
  const handleNoteDrop = (updatedNote) => {
    actionHistory.push(droppedNotes.findLast((note) => note.id === updatedNote))
    setDroppedNotes((prevNotes) =>
      prevNotes.map((note) =>
  ...
  const handleSliderDelete = (sliderId) => {
    actionHistory.push(droppedSliders.findLast((slider) => slider.id === sliderId))
    setDroppedSliders((prevSliders) =>
  ...
  const undoDeleted = (itemId, setFunc) => {
     ...
     * @param setFunc -> the function which sets the item type | e.g. setDroppedStickers for stickers
     */
    setFunc((list) =>
  ...
  const btnUndo = () => {
    ...
    if ("icon" in lastAction) { //Checks is about a sticker (has field icon)
      if ("x" in lastAction) { 
        ...
        let sticker = droppedStickers.find((sticker) => sticker.id === lastAction.id )
        if (sticker) {
          ...
          setDroppedStickers([...droppedStickers, lastAction])
        }
      }
      ...
      if ("x" in lastAction) {
        let slider = droppedStickers.find((slider) => slider.id === lastAction.id )
        if (slider) {
          handleSliderDrop(lastAction)
        }
        else {
          setDroppedSliders([...droppedSliders, lastAction])
        }
      }
        ...
        if (!lastAction.creation) { 
          //If field X exists, it was a delete action or move action
          let note = droppedNotes.find((note) => note.id === lastAction.id )
          if (note) {
            //If the sticker still exists on the page, then it was a move action
            ...
            //If the sticker does not exist on the page then it is a create action
            setDroppedNotes([...droppedNotes, lastAction])
          }
        }
        else {
  ...
  return (
    <div className="direct-work" onClick={handleScreenClick}>
      <div className="direct-work-header">
        <h1 className="direct-work-title">
          <span className='direct-work-title-line1'>{t('About')} {avatar.name}</span>
          <span className='direct-work-title-line2'>{t('this_is_what_is_important_to_me')}</span>
        </h1>
      </div>
      <div className="direct-work-help-button" onClick={handleHelpButtonClick}></div>
      <div className="direct-work-avatar-display-container" ref={drop} style={{ position: 'relative'}}>

        {/* CANVAS FOR DRAWING */}
        <DrawingCanvas forwardRef={getCanvas} storagePath={userDrawingStorage} handleStroke={handleStroke} pencilInfo={selectedPencil} key={selectedPencil} isActive={isDrawingMode}>
      ...
      <div className="direct-work-navigation-button direct-work-navigation-button-left" onClick={handleLeftButtonClick}></div>
      <div className="direct-work-navigation-button direct-work-navigation-button-right" onClick={handleRightButtonClick}></div>
...
export default DirectWorkScreen;